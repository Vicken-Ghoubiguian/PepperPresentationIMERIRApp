<?xml version="1.0" encoding="UTF-8" ?>
<Package name="PepperPresentationIMERIRApp" format_version="4">
    <Manifest src="manifest.xml" />
    <BehaviorDescriptions>
        <BehaviorDescription name="behavior" src="behavior_1" xar="behavior.xar" />
    </BehaviorDescriptions>
    <Dialogs />
    <Resources>
        <File name="IoT" src="boxes_images/IoT.png" />
        <File name="admissions" src="boxes_images/admissions.png" />
        <File name="alternance" src="boxes_images/alternance.png" />
        <File name="chapeau_diplome" src="boxes_images/chapeau_diplome.png" />
        <File name="cybersecurite" src="boxes_images/cybersecurite.png" />
        <File name="erasmus+" src="boxes_images/erasmus+.png" />
        <File name="erasmus+_colleges" src="boxes_images/erasmus+_colleges.png" />
        <File name="fablab" src="boxes_images/fablab.png" />
        <File name="first contact" src="boxes_images/first contact.png" />
        <File name="formulaire" src="boxes_images/formulaire.PNG" />
        <File name="imerir-home-discover-school" src="boxes_images/imerir-home-discover-school.png" />
        <File name="imerir_alumni" src="boxes_images/imerir_alumni.png" />
        <File name="imerir_quesaquo" src="boxes_images/imerir_quesaquo.png" />
        <File name="inscriptions" src="boxes_images/inscriptions.png" />
        <File name="intelligence_artificielle" src="boxes_images/intelligence_artificielle.png" />
        <File name="la_poste" src="boxes_images/la_poste.png" />
        <File name="nous_contacter" src="boxes_images/nous_contacter.png" />
        <File name="pepper_au_revoir" src="boxes_images/pepper_au_revoir.png" />
        <File name="robotique_d_innovation" src="boxes_images/robotique_d_innovation.png" />
        <File name="telephone" src="boxes_images/telephone.png" />
        <File name="tie" src="boxes_images/tie.png" />
        <File name="icon" src="icon.png" />
        <File name="choice_sentences" src="behavior_1/Aldebaran/choice_sentences.xml" />
        <File name="nos_formations" src="boxes_images/nos_formations.png" />
        <File name="pepper_choices" src="boxes_images/pepper_choices.png" />
        <File name="ARE" src="boxes_images/ARE.png" />
        <File name="infos_supplementaires_fablab" src="boxes_images/infos_supplementaires_fablab.png" />
        <File name="prepa" src="boxes_images/prepa.png" />
        <File name="license_info_rob" src="boxes_images/license_info_rob.png" />
        <File name="app_background_image" src="html/images/app_background_image.png" />
        <File name="ARE" src="html/images/boxes_images/ARE.png" />
        <File name="IoT" src="html/images/boxes_images/IoT.png" />
        <File name="admissions" src="html/images/boxes_images/admissions.png" />
        <File name="alternance" src="html/images/boxes_images/alternance.png" />
        <File name="chapeau_diplome" src="html/images/boxes_images/chapeau_diplome.png" />
        <File name="cybersecurite" src="html/images/boxes_images/cybersecurite.png" />
        <File name="erasmus+" src="html/images/boxes_images/erasmus+.png" />
        <File name="erasmus+_colleges" src="html/images/boxes_images/erasmus+_colleges.png" />
        <File name="fablab" src="html/images/boxes_images/fablab.png" />
        <File name="first contact" src="html/images/boxes_images/first contact.png" />
        <File name="formulaire" src="html/images/boxes_images/formulaire.PNG" />
        <File name="imerir-home-discover-school" src="html/images/boxes_images/imerir-home-discover-school.png" />
        <File name="imerir_alumni" src="html/images/boxes_images/imerir_alumni.png" />
        <File name="imerir_quesaquo" src="html/images/boxes_images/imerir_quesaquo.png" />
        <File name="infos_supplementaires_fablab" src="html/images/boxes_images/infos_supplementaires_fablab.png" />
        <File name="inscriptions" src="html/images/boxes_images/inscriptions.png" />
        <File name="intelligence_artificielle" src="html/images/boxes_images/intelligence_artificielle.png" />
        <File name="la_poste" src="html/images/boxes_images/la_poste.png" />
        <File name="license_info_rob" src="html/images/boxes_images/license_info_rob.png" />
        <File name="nos_formations" src="html/images/boxes_images/nos_formations.png" />
        <File name="nous_contacter" src="html/images/boxes_images/nous_contacter.png" />
        <File name="pepper_au_revoir" src="html/images/boxes_images/pepper_au_revoir.png" />
        <File name="pepper_choices" src="html/images/boxes_images/pepper_choices.png" />
        <File name="prepa" src="html/images/boxes_images/prepa.png" />
        <File name="robotique_d_innovation" src="html/images/boxes_images/robotique_d_innovation.png" />
        <File name="telephone" src="html/images/boxes_images/telephone.png" />
        <File name="tie" src="html/images/boxes_images/tie.png" />
    </Resources>
    <Topics />
    <IgnoredPaths />
    <Translations auto-fill="fr_FR">
        <Translation name="translation_fr_FR" src="translations/translation_fr_FR.ts" language="fr_FR" />
    </Translations>
</Package>
